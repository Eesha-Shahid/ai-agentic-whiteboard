        ...
        <>
          <div className="relative">
            ...
            >
              <ToolbarIconWithBar
                icon={Palette}
                color={selectedElement.strokeColor || "#1e1e1e"}
                barWidth="thick"
              />
            </button>
            {openPanel === "textColor" && (
              <ColorPopover
                label="Text color"
                colors={COLORS}
                active={selectedElement.strokeColor}
                onSelect={(c) => onUpdateElement({ strokeColor: c })}
                shape="circle"
              />
            )}
          </div>
            ...
            {openPanel === "align" && (
              <div className="absolute left-1/2 top-10 z-50 flex -translate-x-1/2 items-center gap-1 rounded-full border bg-white p-1 shadow-xl">
                <button
                  ...
                  className={`flex h-8 w-8 items-center justify-center rounded-full ${selectedElement.textAlign === "left" ? "bg-blue-50 text-blue-600" : "hover:bg-slate-100"}`}
                >
                  <AlignLeft size={16} />
                </button>
                <button
                  ...
                  className={`flex h-8 w-8 items-center justify-center rounded-full ${!selectedElement.textAlign || selectedElement.textAlign === "center" ? "bg-blue-50 text-blue-600" : "hover:bg-slate-100"}`}
                >
                  <AlignCenter size={16} />
                </button>
                <button
                  ...
                  className={`flex h-8 w-8 items-center justify-center rounded-full ${selectedElement.textAlign === "right" ? "bg-blue-50 text-blue-600" : "hover:bg-slate-100"}`}
                >
                  <AlignRight size={16} />
                </button>
              </div>
          ...
          <div className="relative">
            ...
            {openPanel === "fill" && (
              <ColorPopover
                ...
                showNoFill
                onNoFill={() =>
                  onUpdateElement({ backgroundColor: "transparent" })
                }
                shape="square"
              />
            )}
                  ...
                  {STROKE_WIDTHS.map((w) => (
                    <button
                      ...
                      className={`flex h-9 flex-1 items-center justify-center rounded-lg border ${
                        (selectedElement.strokeWidth || 2) === w.value
                          ? "border-blue-400 bg-blue-50"
                          : "border-slate-200 hover:bg-slate-50"
                      }`}
                    >
                      <div
                        className="w-8 border-t-2 border-slate-700"
                        style={{ borderTopWidth: w.value }}
                      />
                    </button>
                  ))}
                </div>
  ...
  if (type === "text") {
    return (
      <MorePanelShell
        ...
        onOpacityChange={(v) => onUpdateElement({ opacity: v })}
      >
        <div className="mb-4">
          ...
          <div className="mt-1.5 flex gap-2">
            <FontPill
              label="Hand"
              active={selectedElement.fontFamily === 1}
              onClick={() => onUpdateElement({ fontFamily: 1 })}
            />
            <FontPill
              label="Normal"
              active={selectedElement.fontFamily === 2}
              onClick={() => onUpdateElement({ fontFamily: 2 })}
            />
            <FontPill
              label="Mono"
              active={selectedElement.fontFamily === 3}
              onClick={() => onUpdateElement({ fontFamily: 3 })}
            />
          </div>
        </div>
        <div className="mb-4 flex gap-4">
          <div className="flex-1">
            ...
            <select
              ...
              onChange={(e) =>
                onUpdateElement({ fontSize: Number(e.target.value) })
              }
          ...
          <div className="mt-1.5 flex gap-1.5">
            {COLORS.map((c) => (
              <ColorSquare
                key={c}
                backgroundColor={c}
                active={selectedElement.strokeColor === c}
          ...
          <div className="mt-1.5 flex items-center gap-1.5">
            {COLORS.map((c) => (
              <ColorSquare
                key={c}
                backgroundColor={c}
                active={selectedElement.backgroundColor === c}
            ...
            <button
              ...
              title="No fill"
              className="flex h-7 w-7 shrink-0 items-center justify-center rounded-md border text-slate-400 hover:bg-slate-50"
            >
              <X size={14} />
            </button>
          </div>
...
function StrokeSection({
  ...
  selectedElement: any;
  onUpdateElement: (p: Record<string, any>) => void;
}) {
    ...
    <div className="mb-1">
      ...
      <div className="mb-2 flex gap-2">
        <StrokeStyle
          ...
          onClick={() => onUpdateElement({ strokeStyle: "solid" })}
        />
        <StrokeStyle
          styleType="dashed"
          selected={strokeStyle === "dashed"}
          onClick={() => onUpdateElement({ strokeStyle: "dashed" })}
        />
        <StrokeStyle
          styleType="dotted"
          selected={strokeStyle === "dotted"}
          onClick={() => onUpdateElement({ strokeStyle: "dotted" })}
        />
      </div>
      <select
        value={strokeWidth}
        onChange={(e) =>
          onUpdateElement({ strokeWidth: Number(e.target.value) })
        }
    ...
    <div className="absolute right-0 top-10 z-50 w-80 rounded-2xl border bg-white p-4 shadow-xl">
      ...
      <div className="flex gap-2">
        <button
          ...
          className="flex flex-1 items-center justify-center gap-1.5 rounded-lg border py-2 text-xs text-slate-500 hover:bg-slate-50"
        >
  ...
  return (
    <div className="absolute left-1/2 top-10 z-50 w-max -translate-x-1/2 rounded-xl border bg-white p-3 shadow-xl">
      ...
      {showNoFill && (
        <button
          onClick={onNoFill}
          className="mt-3 w-full rounded-lg border py-1.5 text-xs text-slate-500 hover:bg-slate-50"
        >
          No fill
        </button>
      )}
...
function StrokeStyle({
  ...
  return (
    <button
      ...
      onClick={onClick}
      title={styleType}
      className={`flex h-9 flex-1 items-center justify-center rounded-lg border transition ${selected ? "border-blue-400 bg-blue-50" : "border-slate-200 hover:bg-slate-50"}`}
    >
      <div
        className={`w-8 border-t-2 border-slate-600 ${styleType === "solid" ? "border-solid" : styleType === "dashed" ? "border-dashed" : "border-dotted"}`}
      />
    </button>
  );
}
...
function FontPill({
  ...
  return (
    <button
      onClick={onClick}
      className={`flex-1 rounded-full border py-2 text-xs ${active ? "border-blue-400 bg-blue-50 text-blue-600 font-medium" : "text-slate-500 hover:bg-slate-50"}`}
    >
      {label}
    </button>
  );
...
function AlignBtn({
  ...
  return (
    <button
      onClick={onClick}
      className={`flex h-9 flex-1 items-center justify-center rounded-lg border ${active ? "border-blue-400 bg-blue-50 text-blue-600" : "border-slate-200 hover:bg-slate-50"}`}
    >
      {children}
    </button>
  );
